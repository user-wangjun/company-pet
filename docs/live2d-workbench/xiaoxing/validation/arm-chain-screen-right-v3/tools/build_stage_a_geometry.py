...
def pose_for_angles(theta1: float, theta2: float):
...
def render_contact_sheet(complete, poses):
    ...
    draw.text((20, 50), "固定 L1/L2；无 Physics；黑线仅为骨架叠加。", font=FONT_BODY, fill=(85, 85, 85))
    box = (250, 215, 512, 680)
...
def render_extremes(complete):
    definitions = [
        ("默认", REST_T1, REST_T2),
        ("主动作目标", TARGET_T1, TARGET_T2),
    ]
    for i, item in enumerate(SKELETON["requiredCombinationExtremes"], start=1):
        definitions.append((f"组合极值 {i}", float(item["theta1"]), float(item["theta2"])))
    board = Image.new("RGB", (1050, 970), "white")
    draw = ImageDraw.Draw(board)
    draw.text((25, 16), "小星｜阶段 A 必要组合极值", font=FONT_TITLE, fill=(20, 20, 20))
    draw.text((25, 52), "主路径之外仍检查 θ1/θ2 范围四角，不靠缩小动作掩盖失败。", font=FONT_BODY, fill=(85, 85, 85))
    box = (250, 200, 512, 720)
    for i, (label, t1, t2) in enumerate(definitions):
        pose = pose_for_angles(t1, t2)
        frame, _ = render_pose(complete, pose)
        draw_pose_bones(frame, pose)
        panel = crop_on_checker(frame, box, 0.52)
        x = 25 + (i % 3) * 340
        y = 105 + (i // 3) * 410
        place_panel(board, panel, (x, y), f"{label}｜θ1={t1:.1f}° θ2={t2:.1f}°")
...
def render_seam_stress_review(complete):
    ...
    definitions = [
        ("默认", pose_for_angles(REST_T1, REST_T2)),
        ("主动作目标", pose_for_angles(TARGET_T1, TARGET_T2)),
...
def write_samples(poses):
    payload = {
        "schemaVersion": 1,
        "motion": "quiet_outward_arm_shift",
        "path": "0_to_1_to_0",
        "physics": False,
        "samples": [],
    }
...
def write_report(report):
    ...
    lines = [
        ...
        "## 工程结果",
        "",
        f"- 41 样本与 4 个组合极值均使用固定 `L1={L1:.6f}px`、`L2={L2:.6f}px`。",
